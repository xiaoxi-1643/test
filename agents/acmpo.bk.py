...
class ACMPOAgent(flax.struct.PyTreeNode):
    ...
    def actor_loss(self, batch, grad_params, rng):
        ...
        # q_values = self.network.select(f'target_critic')(
        #     batch['observations'], actions=batch_actions)
        # if self.config['q_agg'] == 'min':
        #     q_values = q_values.min(axis=0)
        # else:
        #     q_values = q_values.mean(axis=0)
        # exp_q = jnp.exp(q_values / 1.0)  # [batch_size,]
        # weights = exp_q / jnp.sum(exp_q, axis=0,
        #                           keepdims=True)
        # weights = weights[:, None]  # [batch_size, 1]
        # weights = jnp.broadcast_to(
        #     weights, (batch_size, self.config["horizon_length"]))  # [batch_size, horizon_length]
        ...
        if self.config["actor_type"] == "distill-ddpg":
            ...
            # noises = jax.random.normal(noise_rng1, (batch_size, action_dim))
            ...
            observations = jnp.repeat(
                batch['observations'][..., None, :], self.config["actor_num_samples"], axis=-2)
            ...
            if self.config['q_agg'] == 'min':
                target_q_values = self.network.select(f'target_critic')(
                    observations, actions=target_flow_actions).min(axis=0)
            else:
                target_q_values = self.network.select(f'target_critic')(
                    observations, actions=target_flow_actions).mean(axis=0)  # [batch_size, actor_num_samples]
            ...
            # noises = jax.random.normal(noise_rng2, (batch_size, action_dim))
            # actor_actions = self.network.select('actor_onestep_flow')(batch['observations'], noises, params=grad_params)
            # distill_loss = jnp.mean((actor_actions - target_flow_actions_max) ** 2)

            max_q = jnp.max(target_q_values, axis=1, keepdims=True)
            exp_q = jnp.exp((target_q_values - max_q) / self.config['temperature'])
            # exp_q = jnp.exp(target_q_values / temperature)
            weights = exp_q / (jnp.sum(exp_q, axis=1, keepdims=True) + 1e-8)
            weights = weights[..., None]  # [batch_size, actor_num_samples, 1]

            # noises = jax.random.normal(noise_rng, (
            #     *batch['observations'].shape[: -len(self.config['ob_dims'])], self.config["actor_num_samples"], action_dim),)
            actor_actions = self.network.select('actor_onestep_flow')(observations, noises, params=grad_params)
            # distill_loss = jnp.mean(jnp.sum(weights * (actor_actions - target_flow_actions) ** 2, axis=tuple(range(1, weights.ndim))))
            distill_loss = jnp.mean(weights * (actor_actions - target_flow_actions) ** 2)
    ...
    def sample_actions(
        ...
        if self.config["actor_type"] == "distill-ddpg":
            ...
            actions = self.network.select(
                f'actor_onestep_flow')(observations, noises)
            actions = jnp.clip(actions, -1, 1)

        elif self.config["actor_type"] == "best-of-n":
            action_dim = self.config['action_dim'] * \
                (self.config['horizon_length']
                 if self.config["action_chunking"] else 1)
            noises = jax.random.normal(rng, (
                    *observations.shape[: -len(self.config['ob_dims'])],
    ...
    def compute_flow_actions(
        ...
        if self.config['encoder'] is not None:
            observations = self.network.select(
                'actor_bc_flow_encoder')(observations)
        actions = noises
        # Euler method.
        for i in range(self.config['flow_steps']):
            t = jnp.full(
                (*observations.shape[:-1], 1), i / self.config['flow_steps'])
...
def get_config():
    ...
    config = ml_collections.ConfigDict(
        dict(
            ...
            # BC coefficient (need to be tuned for each environment).
            alpha=100.0,
            temperature=0.1,
            ...
            horizon_length=ml_collections.config_dict.placeholder(
                int),  # will be set
